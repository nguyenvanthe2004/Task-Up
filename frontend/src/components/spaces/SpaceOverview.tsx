...
const SpaceOverview: React.FC = () => {
      ...
      try {
        ...
        if (taskRes.status === "fulfilled") {
          ...
          const filtered = all.filter(
            (t) =>
              String((t.list as any)?.category?.space?.id) === String(spaceId),
          );
          setTasks(filtered);
        ...
        if (actRes.status === "fulfilled") {
          ...
          const filtered = all.filter(
            (a) => String((a as any)?.spaceId) === String(spaceId),
          );
          setActivities(
            filtered.length > 0 ? filtered : actRes.value.data.slice(0, 5),
          );
        }
  ...
  const isEmpty = !loading && totalTasks === 0 && attachments.length === 0;
            ...
            {isEmpty ? (
              <div className="flex flex-col items-center justify-center py-32 text-center">
                ...
                <p className="text-sm text-on-surface-variant max-w-xs">
                  No tasks or files yet. Start by creating a list or inviting
