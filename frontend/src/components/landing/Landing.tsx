...
const TaskCard: React.FC<{
  ...
  progress: number; priority: "High" | "Medium" | "Low"; avatarColors: string[];
...
const FeatureCard: React.FC<{
  icon: React.ReactNode; title: string; desc: string; iconBg: string; iconColor: string;
...
const TestimonialCard: React.FC<{
  ...
  <div className="bg-white border border-slate-200 rounded-2xl p-6 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
    ...
    <div className="flex items-center gap-3">
      <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold text-white flex-shrink-0" style={{ background: avatar }}>
        {name.charAt(0)}
      </div>
      <div>
...
const Landing: React.FC = () => {
  ...
  // Xử lý invite token ngầm, không render gì thêm
  useAcceptInvite();
    ...
    <div className="bg-white font-sans antialiased overflow-x-hidden">
      ...
      <section className="relative pt-36 pb-28 px-6 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-indigo-50 border border-indigo-100 text-indigo-600 px-4 py-1.5 rounded-full mb-8 text-[11px] font-bold uppercase tracking-widest shadow-sm">
            <Sparkles size={11} /> The smarter way to manage tasks
          </div>
          <h1 className="text-5xl md:text-[72px] font-black tracking-tight text-slate-900 leading-[1.02] mb-6 max-w-4xl mx-auto">
            Organize your work.{" "}
            <span className="relative inline-block">
              <span className="bg-gradient-to-r from-violet-600 via-indigo-600 to-blue-600 bg-clip-text text-transparent">Crush your goals.</span>
          ...
          <div className="flex flex-wrap justify-center gap-6 text-sm text-slate-400 font-medium mb-20">
            {["Free forever", "No credit card required", "Set up in 2 minutes"].map((text) => (
              <div key={text} className="flex items-center gap-1.5">
                <CheckCircle2 size={14} className="text-emerald-500" /><span>{text}</span>
              </div>
            ))}
          </div>
          ...
          <div className="relative max-w-5xl mx-auto">
            <div className="rounded-2xl border border-slate-200 bg-white shadow-[0_24px_80px_rgba(99,102,241,0.12)] overflow-hidden">
              <div className="bg-slate-50 border-b border-slate-200 flex items-center gap-2 px-4 py-3">
                ...
                <div className="flex-1 flex justify-center">
                  <div className="bg-white border border-slate-200 rounded-lg px-8 py-1 text-[11px] text-slate-400 font-medium">app.taskup.io/dashboard</div>
                </div>
              </div>
            ...
            <div className="absolute -left-14 top-1/4 hidden lg:block drop-shadow-2xl">
              <TaskCard title="Redesign landing page UI" tag="Design" tagColor="#6366f1" progress={72} priority="High" avatarColors={["#6366f1","#22d3ee"]} />
            </div>
            <div className="absolute -right-12 top-1/3 hidden lg:block drop-shadow-2xl">
              <TaskCard title="Write API documentation" tag="Dev" tagColor="#10b981" progress={45} priority="Medium" avatarColors={["#f472b6","#fb923c","#6366f1"]} />
            </div>
      ...
      <section className="bg-slate-50 border-y border-slate-100 py-12 mt-12">
        <div className="max-w-6xl mx-auto px-6">
          <p className="text-center text-[10px] font-bold uppercase tracking-[0.3em] text-slate-400 mb-8">Trusted by 50,000+ teams worldwide</p>
          <div className="flex flex-wrap justify-center items-center gap-12 md:gap-20">
            {["SAMSUNG","Booking.com","IBM","Shopify","Netflix"].map((brand) => (
              <span key={brand} className="text-xl font-black tracking-tighter text-slate-300 hover:text-slate-500 transition-colors duration-300 cursor-default select-none">{brand}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-28">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-4 leading-tight tracking-tight">
            ...
            <span className="bg-gradient-to-r from-violet-600 to-indigo-600 bg-clip-text text-transparent">work smarter</span>
          </h2>
          <p className="text-slate-500 text-lg max-w-xl mx-auto font-medium">From personal to-dos to large team projects — TaskUp handles it all with ease.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {[
            { icon: <ListTodo size={22} />, title: "Task Management", desc: "Create, assign, and track tasks with Kanban boards, lists, and intuitive calendars. Prioritize work intelligently.", iconBg: "bg-indigo-50", iconColor: "text-indigo-600" },
            { icon: <Users size={22} />, title: "Team Collaboration", desc: "Work together in real time. Comment, mention, share files, and update progress — all within tasks.", iconBg: "bg-violet-50", iconColor: "text-violet-600" },
            { icon: <BarChart3 size={22} />, title: "Reports & Analytics", desc: "Visual dashboards help you understand team productivity. Track progress, time, and identify bottlenecks fast.", iconBg: "bg-sky-50", iconColor: "text-sky-600" },
            { icon: <Bell size={22} />, title: "Smart Notifications", desc: "Get timely alerts on deadlines, task updates, and team activity. Never miss anything important again.", iconBg: "bg-amber-50", iconColor: "text-amber-600" },
            { icon: <Calendar size={22} />, title: "Calendar & Timeline", desc: "View all your work on a calendar or Gantt chart. Plan sprints and manage deadlines with ease.", iconBg: "bg-emerald-50", iconColor: "text-emerald-600" },
            { icon: <Shield size={22} />, title: "Enterprise Security", desc: "Your data is end-to-end encrypted. Control access permissions for every member and project in detail.", iconBg: "bg-rose-50", iconColor: "text-rose-600" },
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-4 tracking-tight">
              Up and running in{" "}
              <span className="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">3 simple steps</span>
            </h2>
            <p className="text-slate-500 text-lg max-w-xl mx-auto font-medium">No complex onboarding. No steep learning curve. Just results.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 relative">
            ...
            {[
              { step: "01", icon: <Target size={24} />, title: "Create a workspace", desc: "Set up a workspace for your team. Invite members and configure access permissions in seconds.", color: "text-indigo-600", bg: "bg-indigo-50", border: "border-indigo-100" },
              { step: "02", icon: <ListTodo size={24} />, title: "Add your tasks", desc: "Create tasks, assign them to teammates, set deadlines, and add tags to organize everything clearly.", color: "text-violet-600", bg: "bg-violet-50", border: "border-violet-100" },
              { step: "03", icon: <TrendingUp size={24} />, title: "Track & deliver", desc: "Update progress, view real-time reports, and celebrate when your team smashes their goals.", color: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-100" },
              ...
              <div key={step} className="relative flex flex-col items-center text-center group">
                <div className={`relative z-10 w-20 h-20 ${bg} border-2 ${border} rounded-2xl flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-300`}>
                  ...
                  <span className="absolute -top-2.5 -right-2.5 w-6 h-6 bg-slate-900 text-white text-[10px] font-black rounded-full flex items-center justify-center">{step}</span>
                </div>
      ...
      <section className="bg-slate-50 border-t border-slate-100 py-28 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-4 tracking-tight">Loved by teams everywhere</h2>
            <p className="text-slate-500 text-lg max-w-lg mx-auto font-medium">Don't take our word for it — here's what our users have to say.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {[
              { quote: "TaskUp completely changed how our team operates. Productivity jumped noticeably within just two weeks of using it.", name: "Sarah Johnson", role: "CTO at Acme Corp", avatar: "#6366f1", rating: 5 },
              { quote: "Beautiful UI, super easy to use, and packed with features. This is the best task management tool I've ever tried.", name: "Michael Chen", role: "Product Manager at Stripe", avatar: "#f472b6", rating: 5 },
              { quote: "We tried many tools before, but TaskUp is the only one that fits seamlessly into our entire team's workflow.", name: "Emily Rodriguez", role: "Founder & CEO at LaunchPad", avatar: "#10b981", rating: 5 },
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white border border-slate-200 rounded-2xl p-8 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
              ...
              <button onClick={() => navigate("/register")} className="w-full py-3 border-2 border-slate-200 rounded-xl text-sm font-bold text-slate-700 hover:border-indigo-300 hover:text-indigo-600 hover:bg-indigo-50 transition-all duration-200">
                Get started free
              </button>
            </div>
            <div className="relative bg-gradient-to-br from-indigo-600 to-violet-600 rounded-2xl p-8 shadow-xl shadow-indigo-500/25 hover:-translate-y-0.5 transition-transform duration-300">
              <div className="absolute top-6 right-6">
                <span className="bg-white/20 text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">Most popular</span>
              </div>
              <div className="mb-6">
              ...
              <ul className="space-y-3 mb-8">
                {["Unlimited members","Unlimited workspaces","Gantt chart & Timeline","Advanced analytics","API access","Priority 24/7 support"].map((f) => (
                  <li key={f} className="flex items-center gap-2.5 text-sm text-white">
                    <CheckCircle2 size={15} className="text-indigo-200 flex-shrink-0" />{f}
                  </li>
                ))}
              </ul>
              <button onClick={() => navigate("/register")} className="w-full py-3 bg-white rounded-xl text-sm font-bold text-indigo-600 hover:bg-indigo-50 transition-colors duration-200 shadow-md">
        ...
        <div className="max-w-5xl mx-auto">
          <div className="relative bg-slate-900 rounded-3xl px-10 py-20 text-center overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-4xl md:text-5xl font-black text-white mb-5 leading-tight tracking-tight">Save one day every week.</h2>
              ...
              <p className="text-slate-600 text-xs mt-6 font-medium">Free forever · No credit card required · Cancel anytime</p>
            </div>
          </div>
        </div>
      </section>
