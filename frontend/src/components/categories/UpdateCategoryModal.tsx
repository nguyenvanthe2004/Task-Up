...
import { callGetCategoryById } from "../../services/category";
...
interface UpdateCategoryModalProps {
  ...
  onClose: () => void;
  onSubmit: (id: number, data: UpdateCategoryFormData) => Promise<void>;
}
...
export default function UpdateCategoryModal({
  ...
  useEffect(() => {
    ...
    const fetchCategory = async () => {
      setFetching(true);
      try {
        const res = await callGetCategoryById(categoryId);
        const cat = res.data.dataValues;
        setValue("name", cat.name ?? "");
        setValue("description", cat.description ?? "");

        const foundIconIdx = ICONS.findIndex((i) => i.label === cat.icon);
        setIconIdx(foundIconIdx >= 0 ? foundIconIdx : 0);

        const foundColor = COLORS.find((c) => c.hex === cat.color);
  ...
  const onValid = async (data: UpdateCategoryFormData) => {
